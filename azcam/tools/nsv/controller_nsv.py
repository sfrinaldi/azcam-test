...
class ControllerNSV(Controller):
    ...
    def __init__(self, tool_id="controller", description=None):
        ...
        # Config data lines read from a file or downloaded - raw format 'parameter=value'
        self.config_data = []
    ...
    def nsv_bin_command(self, command):
        """
        Send binary command to the NSV controller.
        """
    ...
    # Note -> Keeping this / don't think there are edits to make for this? (SFR)
    ...
    # Keeping this / don't think edits are needed (SFR)
    def reboot(self):
        """
        Send REBOOT command.
        """
    ...
    def start_exposure(self, wait=1):
        ...
        self.nsv_status = self.EXP_UNKNOWN
        ...
        if wait == 0:
            self.nsv_status = self.EXP_EXPOSE
            return
    ...
    def download_config(self):
        ...
        while endCfg != 1:
            ...
            cnt += 1
            if len(reply) > 0:
                self.ConfigNSV.append(reply[1])
            else:
                endCfg = 1

        self.ConfigNSVCnt = len(self.ConfigNSV)
